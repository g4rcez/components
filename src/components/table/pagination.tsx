...
export const Pagination = (pagination: TablePagination) => {
    ...
    return <footer className="flex px-1 py-2 items-center justify-center gap-4 lg:justify-between flex-wrap lg:flex-nowrap">
        <p>
            ...
            {pagination.onChangeSize && Array.isArray(pagination.sizes) ?
                <Fragment>
                    <select value={pagination.size} className="cursor-pointer bg-transparent" onChange={e => {
                        pagination.onChangeSize?.(Number(e.target.value))
                    }}>
                        {pagination.sizes.map(value => <option value={value}>{value}</option>)}
                    </select> per page
                </Fragment>
                : null}
        </p>
        <nav>
            <ul className="flex items-center gap-2">
                {pagination.current > 1 ? <li>
                    <Render href="previous" className="">Previous</Render>
                </li> : null}
                {pageNavigation.map(x => {
                    ...
                    return <Fragment>
                        {typeof x === "string" ?
                            ...
                            <li key={`pagination-${x}`} >
                                <Render href={x} className={
                                    `cursor-pointer px-3 py-1 transition-colors border-b-2 hover:text-primary-subtle hover:border-primary-subtle proportional-nums ${x === pagination.current ? "text-primary border-primary" : "border-transparent"}`
                                }>
                                    {x}
                                </Render>
                            </li>}
                ...
                {hasNext ? <li>
                    <Render href="next" className="">Next</Render>
                </li> : null}
            </ul>
        </nav>
    </footer>
