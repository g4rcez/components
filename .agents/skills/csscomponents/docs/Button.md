...
`--var-button-icon-font-size` retains its legacy name but controls icon geometry (including SVG dimensions), so its default now derives from spacing. Ordinary button text still uses the independent typography tokens.
