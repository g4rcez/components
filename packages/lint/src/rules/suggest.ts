...
export function nearestColorTokens(lab: Lab, tokens: Map<string, Lab>, role: ColorRole, limit = 2) {
    ...
    const prefer = role === "text" ? isForeground : (n: string) => !isForeground(n);
...
export function nearestSteps(px: number, scale: Map<string, number>, limit = 2) {
    return [...scale]
        .map(([name, value]) => ({
            name,
            px: value,
...
export function formatSteps(steps: { name: string; px: number }[], toClass: (name: string) => string) {
