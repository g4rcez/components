...
// Oxlint supplies `physicalFilename`; the getter keeps older integrations working.
...
function interpolate(text: string, data: Record<string, unknown>) {
    return text.replace(PLACEHOLDER, (match, key: string, fallback: string | undefined) => {
...
export function reporter(context: any, messages: Record<string, string>, options: { rule?: string; message?: unknown } = {}) {
