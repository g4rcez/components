...
const isFontSizeAttr = (attr: string) => attr === "text" || attr.endsWith("-text") || attr.startsWith("text-");

const isBorderWidthAttr = (attr: string) => attr === "border" || attr.endsWith("-border");

const isRadiusAttr = (attr: string) => attr === "radius" || attr.endsWith("-radius");
...
const isComponentToken = (value: string) => componentPrefixes.some((p) => value === p || value.startsWith(`${p}-`));
...
const isFontSizeValue = (value: string) => {
    ...
    if (value.startsWith("typography-")) return fontSizeTokens.has(value.slice("typography-".length));
    return false;
};

const isBorderWidthValue = (value: string) => borderWidthTokens.has(value);

const isTextColorValue = (value: string) => !isFontSizeValue(value) && isComponentToken(value);

const isBorderColorValue = (value: string) => !isBorderWidthValue(value) && isComponentToken(value);
...
const wrap = (ids: readonly string[], match: (v: string) => boolean) => Object.fromEntries(ids.map((id) => [id, [{ [id]: [match] }]]));
...
export const isChildVisible = (container: HTMLElement, child: HTMLElement, partial: boolean = true): boolean => {
