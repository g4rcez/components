...
const List = forwardRef(function VirtualList(props: any, ref: any) {
  return (
    <motion.ul
      {...props}
      ref={ref as any}
      className="overscroll-contain w-full max-h-96 rounded-lg"
    >
      ...
      <InputField
        ...
        right={
          <span className="flex gap-0.5 items-center">
            ...
            {value ? (
              <button type="button" onClick={onClose} className="p-2 transition-colors md:p-1 link:text-danger">
          ...
          {open ? (
            <FloatingFocusManager modal guards returnFocus={false} context={context} initialFocus={-1} visuallyHiddenDismiss>
              <motion.div
                ...
                className={css("overscroll-contain p-0 m-0 max-h-80 list-none rounded-t-lg rounded-b-lg border ease-in-out isolate z-floating origin-[top_center] border-floating-border bg-floating-background text-foreground shadow-floating", isTopPlacement ? "origin-[bottom_center]" : "origin-[top_center]")}
                onAnimationComplete={() => {
                  if (!open) return setH(0);
