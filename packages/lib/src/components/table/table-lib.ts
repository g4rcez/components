...
export const getLabel = <T extends POJO>(col: Col<T>) =>
    col.headerLabel ?? (typeof col.thead === "function" ? (col.id as string) : col.thead) ?? (col.id as string);
...
export const createOptionCols = <T extends POJO>(cols: Col<T>[]): OptionProps[] =>
    cols.map((opt) => {
        ...
        return {
            ...
            label: typeof label === "string" || typeof label === "number" || typeof label === "bigint" ? String(label) : (opt.id as string),
        };
    });
...
export type TableHeaderCreatorProps = {
    Properties: React.ComponentType;
};

type THead = React.ReactNode | React.ComponentType<TableHeaderCreatorProps>;
...
const mergeCols = <T extends POJO>(cols: Col<T>[], saved?: Col<T>[]) => {
    ...
    const map = new Map(cols.map((col) => [col.id, col]));
    const restored: Col<T>[] = [];
    for (const savedCol of saved) {
        const original = map.get(savedCol.id);
        if (!original) continue;
        map.delete(savedCol.id);
        restored.push({ ...original, visible: savedCol.visible ?? original.visible });
    }
    return restored.concat(cols.filter((col) => map.has(col.id)));
};
