...
export const TimelineItem = (props: PropsWithChildren) => (
    <li data-component="timeline-item" className="__display-timeline__slot-1" role="listitem">
        <span aria-hidden="true" className="__display-timeline__slot-2" />
        <div className="__display-timeline__slot-3">{props.children}</div>
    </li>
);

TimelineItem.Icon = function TimelineIcon(props: PropsWithChildren<ComponentProps<"header">>) {
    return (
        <header {...props} className={css("__display-timeline__slot-4", props.className)}>
            {props.children}
        </header>
    );
};
...
TimelineItem.Body = function TimelineItemBody<T extends ElementType = "section">(props: PropsWithChildren<PolymorphicProps<object, T>>) {
    return (
        <Polymorph {...props} className={css("__display-timeline__slot-5", props.className)}>
            {props.children}
        </Polymorph>
    );
};
...
TimelineItem.Right = function TimelineItemRight<T extends ElementType = "button">(props: PolymorphicProps<object, T>) {
    return (
        <footer className="__display-timeline__slot-6">
