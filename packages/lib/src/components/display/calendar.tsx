...
const formatMonth = (d: Date, locale?: Locales) =>
...
const onChangeUsingKeyboard = {
  ArrowLeft: (date: Date, duration: "days" | "month") =>
    duration === "days" ? sub(date, { days: 1 }) : sub(date, { months: 1 }),
  ArrowRight: (date: Date, duration: "days" | "month") =>
    duration === "days" ? add(date, { days: 1 }) : add(date, { months: 1 }),
  ArrowUp: (date: Date, duration: "days" | "month") =>
    duration === "days" ? sub(date, { weeks: 1 }) : sub(date, { years: 1 }),
  ArrowDown: (date: Date, duration: "days" | "month") =>
    duration === "days" ? add(date, { weeks: 1 }) : add(date, { years: 1 }),
  ...
  const select = () => {
    if (origin?.dataset.focustrap) {
      const el = root.current?.querySelector(
        `button[data-focustrap="${origin?.dataset.focustrap}"]`,
      ) as HTMLButtonElement;
      return setTimeout(() => el?.focus({ preventScroll: true }), delay);
    }
    if (root.current) {
      const element = root.current.querySelector<HTMLButtonElement>(
        `button[data-date="${d}"]`,
      );
...
const CalendarBody = (props: CalendarBodyProps) => {
  ...
  return (
    <motion.tbody
      ...
      className={css(props.styles?.week)}
    >
      {props.zip.map((week, index) => {
        const weekClassName = Is.function(props.styles?.week)
          ? props.styles?.week(week)
          : props.styles?.week;
        return (
          <tr key={`week-${week.length}-${index}`} className={weekClassName}>
            {week.map((day) => {
              ...
              const isSelected = props.rangeMode
                ? key === props.range?.to?.toISOString() ||
                  key === props.range?.from?.toISOString()
                : key ===
                  (props.date
                    ? startOfDay(props.date).toISOString()
                    : undefined);
              ...
              const isInRange = props.rangeMode
                ? inRange(props.range?.from, day, props.range?.to)
                : false;
              return (
                <td
                  key={key}
                  ...
                  className={css(
                    ...
                    Is.function(props.styles?.dayFrame)
                      ? props.styles?.dayFrame(day)
                      : props.styles?.dayFrame,
                  )}
                >
                  <button
                    type="button"
                    ...
                    className={css(
                      ...
                      isInRange && props.markRange
                        ? "size-calendar-day-size border border-dashed border-card-border"
                        : "",
                      Is.function(props.styles?.day)
                        ? props.styles?.day(day)
                        : props.styles?.day,
                    )}
                  >
                    <div></div>
                    {day.getDate()}
                    {isSelected &&
                    props.stateRange.from?.toISOString() === key ? (
                    ...
                    {isSelected &&
                    props.stateRange.to?.toISOString() === key ? (
                      <span className="absolute -top-2 left-0 h-full w-full">
                        <span className="text-calendar-cell-text text-foreground">
...
export const Calendar = ({
  ...
  const [state, dispatch] = useReducer(
    ...
    (get) => ({
      ...
      onKeyDown: (e: React.KeyboardEvent<HTMLTableSectionElement>) => {
        ...
        if (key in onChangeUsingKeyboard) {
          ...
          const date = Is.keyof(onChangeUsingKeyboard, key)
            ? onChangeUsingKeyboard[key](prev, e.shiftKey ? "month" : "days")
            : null;
          if (date !== null) {
    ...
    }),
    {
      ...
      postMiddleware: [
        (state, _, args) => {
          const isValidMethod =
            args.method === "onChangeMonth" ||
            args.method === "previousMonth" ||
            args.method === "nextMonth";
        ...
        (state, _, args) => {
          const isValidMethod =
            args.method === "onSelectDate" ||
            args.method === "setToday" ||
            args.method === "onKeyDown";
  ...
  const defer = useDebounce(
    (y: string) => dispatch.date((prev) => setYear(new Date(prev), +y)),
    1200,
  );
    ...
    <MotionConfig transition={transition}>
      <div
        ...
        className={css(
          ...
          Is.function(styles?.calendar)
            ? styles?.calendar(allDaysOfMonth)
            : styles?.calendar,
        )}
      >
