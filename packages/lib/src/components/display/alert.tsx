...
const themeVariants = {
  theme: {
    primary:
      "bg-alert-primary-bg text-alert-primary-text border-alert-primary-border",
    danger:
    ...
    secondary:
      "bg-alert-secondary-bg text-alert-secondary-text border-alert-secondary-border",
...
const alertVariants = cva(
  "text-typography-sm relative block w-full rounded-alert-radius border p-alert-p",
  {
...
export const Alert: <T extends React.ElementType = "div">(
  props: AlertProps<T>,
) => React.ReactNode = forwardRef(function Alert(
        ...
        >
          <div className="flex flex-col gap-alert-gap">
            ...
            <div className="flex items-center gap-alert-gap">
              ...
              {Icon}
              {props.title ? (
                <h4 className="tracking-3 text-typography-lg text-balance font-semibold">
                  {props.title}
                </h4>
              ) : (
