    ...
    <header className="flex flex-col gap-page-calendar-header-gap">
      <div className="flex items-center justify-between gap-page-calendar-header-gap">
        <div className="flex items-center gap-page-calendar-date-gap">
          ...
          >
            <span className="font-light uppercase leading-none">
              {formatMonthShort(currentDate, locale)}
            </span>
            <span className="text-page-calendar-title-text font-medium leading-none">
          ...
          <div>
            <h1
              aria-live="polite"
              aria-atomic="true"
              className="text-page-calendar-title-text font-bold leading-tight"
            >
              {formatMonthYear(currentDate, locale)}
            </h1>
            <span className="text-page-calendar-week-label-text text-muted-foreground">
              {t.pageCalendarWeekLabel(weekNum)}
            </span>
          </div>
        </div>
        <nav
          aria-label={t.pageCalendarNavigation}
          className="flex items-center gap-page-calendar-nav-gap"
        >
          <div className="flex items-center gap-page-calendar-nav-btn-gap">
            ...
            <Button
              size="small"
              title={t.pageCalendarNext}
              aria-label={t.pageCalendarNext}
              theme="ghost-muted"
              onClick={handleNext}
            >
      ...
      {filterArea ??
        (filters.length > 0 && (
          <div
            ...
            className="flex flex-wrap items-center gap-page-calendar-filter-gap"
          >
            <CalendarIcon
              size={14}
              ...
              aria-hidden="true"
            />
            <span className="mr-page-calendar-nav-mr text-page-calendar-filter-text text-muted-foreground">
              {t.pageCalendarFilter}
            </span>
            {filters.map((filter) => (
              <Tag
                as="button"
