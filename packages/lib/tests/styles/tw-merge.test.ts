...
describe("css() recognises component tokens", () => {
    ...
    it("dedupes rounded tokens", () => {
        expect(css("rounded-lg", "rounded-card-radius")).toBe("rounded-card-radius");
        expect(css("rounded-t-lg", "rounded-t-modal-radius")).toBe("rounded-t-modal-radius");
    });
    ...
    it("keeps unrelated utilities untouched", () => {
        expect(css("flex p-stats-icon-p text-foreground")).toBe("flex p-stats-icon-p text-foreground");
    });

    it("dedupes form input spacing and radius tokens", () => {
        expect(css("p-2", "p-input-padding-x")).toBe("p-input-padding-x");
        expect(css("rounded-md", "rounded-input-radius")).toBe("rounded-input-radius");
    ...
    it("dedupes selection control tokens", () => {
        ...
        expect(css("text-sm", "text-switch-label-text")).toBe("text-switch-label-text");
    });

    it("dedupes slider thumb and file-upload tokens", () => {
        expect(css("size-5", "size-slider-thumb-size")).toBe("size-slider-thumb-size");
    ...
    it("dedupes new component token group spacings", () => {
        ...
        expect(css("bottom-4", "bottom-toolbar-bottom")).toBe("bottom-toolbar-bottom");
        expect(css("w-80", "w-wizard-w")).toBe("w-wizard-w");
        expect(css("min-h-32", "min-h-page-calendar-cell-min-h")).toBe("min-h-page-calendar-cell-min-h");
    });

    it("dedupes typography fillers against tailwind defaults", () => {
        expect(css("text-sm", "text-typography-sm")).toBe("text-typography-sm");
        expect(css("text-base", "text-typography-base")).toBe("text-typography-base");
        expect(css("text-lg", "text-typography-lg")).toBe("text-typography-lg");
        expect(css("text-[10px]", "text-page-calendar-hour-text")).toBe("text-page-calendar-hour-text");
    });

    it("dedupes table rounded corner tokens", () => {
        expect(css("rounded-tl-lg", "rounded-tl-table-radius")).toBe("rounded-tl-table-radius");
    });

    it("dedupes spinner border-width token", () => {
        expect(css("border-4", "border-spinner-border")).toBe("border-spinner-border");
    });

    it("dedupes compound -radius tokens (modal resizer, sheet pill, notification close)", () => {
        expect(css("rounded-lg", "rounded-modal-resizer-radius")).toBe("rounded-modal-resizer-radius");
        expect(css("rounded-lg", "rounded-modal-sheet-pill-radius")).toBe("rounded-modal-sheet-pill-radius");
        expect(css("rounded-full", "rounded-notification-close-radius")).toBe("rounded-notification-close-radius");
    });
});
