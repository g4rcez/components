      ...
      <AnimatePresence>
        {date !== undefined && date.toISOString() in eventDates ? (
          ...
          >
            ...
            >
                <Card title="Calendar + Bookmark" className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    <Calendar
                        date={date}
                        changeOnlyOnClick
                        onChange={setDate}
                        ...
                        styles={{
                            week: "border-b border-card-border",
                            weekDay: "h-16 border-b border-l first:border-l-transparent border-t border-card-border",
                            dayFrame: "h-16 border-card-border border-l border-r first:border-l-transparent last:border-r-transparent",
                        }}
                        RenderOnDay={({ date }) =>
                            date.toISOString() in dates ? (
                                <div className="absolute right-0 top-0 flex aspect-square size-5 items-center justify-center rounded-full bg-warn text-sm text-warn-foreground">
                    ...
                    <AnimatePresence>
                        {date !== undefined ? (
                            <motion.div className="h-full w-full">
                                <header className="pb-4 text-center text-2xl font-medium">{date.toLocaleDateString(undefined, options)}</header>
    ...
    <Card title="Range calendar" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p>Selected To: {range.to?.toLocaleDateString()}</p>
            ...
            >
                <Card title="Range calendar" className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    <Calendar
                        markRange={false}
                        rangeMode
                        range={range}
                        ...
                        RenderOnDay={({ date }) =>
                            date.toISOString() in dates ? (
                                <div className="absolute bottom-1 right-1 flex aspect-square size-5 items-center justify-center rounded-full bg-warn text-sm text-warn-foreground">
                                    {dates[date.toISOString()].length}
                                </div>
                            ) : null
                        }
