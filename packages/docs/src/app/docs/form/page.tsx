...
const oabMask = (str: string) => {
    ...
    if (numbers.length === 6) return [/\d/, /\d/, /\d/, /\d/, /\d/, /\d/, "/", /[A-Za-z]/, /[A-Za-z]/];
...
const schema = z.object({
    ...
    items: z.array(z.object({ document: z.string().min(1), name: z.string().min(1) })),
    oab: z.array(
        z.object({
            document: z
                .string()
...
export default function FormPage() {
    ...
    return (
        <DocsLayout
            title="useForm"
            section="form"
            description="Form elements are useful when you need to accept user data entries. This library provides a useForm hook that integrates with Zod to create forms as fast as possible."
        >
            <ComponentDemo
                title="Dynamic Form with Zod Validation"
                description="Demonstrates a comprehensive form using `useForm` hook with Zod schema validation, including various input types, dynamic fields, and submission handling."
                code={`"use client";
...
`}
            >
                <Card>
                    <form name="form" id="form" onSubmit={form.onSubmit(onSubmit)} onInvalid={form.onInvalid((e) => console.log(e))} />
                    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                        ...
                        />
                        <DatePickerControlled />
                        <Switch {...form.checkbox("agree")} container="md:col-span-2 lg:col-span-3">
                            Accept the terms - Switch
                        </Switch>
                        <Checkbox {...form.checkbox("boolean")} container="md:col-span-2 lg:col-span-3">
                            Accept the terms - Checkbox
                        </Checkbox>
                    </div>
                    <ul className="my-4 space-y-4">
                        {items.map((item) => (
                            <li key={item} className="flex flex-nowrap items-baseline gap-8 align-baseline">
                                <Input {...form.input(`items[${item}].name`)} title={`Name - ${item}`} />
                                <Input {...form.input(`items[${item}].document`, { mask: "cpf" })} title={`Documento - ${item}`} />
                                <Input {...form.input(`oab[${item}].document`, { mask: oabMask })} title={`Oab - ${item}`} />
                            </li>
                        ))}
                        <li>
                            <Button onClick={() => setItems((prev) => prev.concat(prev.at(-1)! + 1))}>Add other item</Button>
                        </li>
                    </ul>
                    <div className="flex items-center">
                        <Button form={form.name} type="submit">
            ...
            <ComponentDemo
                title="Form Debug State"
                description="Displays the current state of the form object, useful for debugging and understanding form data flow."
                code={`"use client";
import { z } from "zod";
...
function FormDebug() {
  const form = useForm(schema, "debug-form");
  const [state, setState] = useState({}); // Example state to show submission

  return (
    <Card title={\`Debug - User: \${form.get("name")}\`}>
      <pre className="mt-10">
