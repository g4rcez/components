...
import { Card, Radiobox } from "@g4rcez/components";
      ...
      <Radiobox name="notifications" value="push">Push notification</Radiobox>
            ...
            >
                <Card title="Basic group">
                    <fieldset className="flex flex-col gap-3">
                        <legend className="mb-2 text-sm font-medium">Notification preference</legend>
          ...
          {plan.charAt(0).toUpperCase() + plan.slice(1)}
            ...
            >
                <Card title="Controlled">
                    <fieldset className="flex flex-col gap-3">
                        <legend className="mb-2 text-sm font-medium">Billing cycle — selected: {selected}</legend>
                        {["monthly", "quarterly", "annually"].map((plan) => (
                            <Radiobox key={plan} name="billing" value={plan} checked={selected === plan} onChange={() => setSelected(plan)}>
                                {plan.charAt(0).toUpperCase() + plan.slice(1)}
                            </Radiobox>
                        ))}
                    </fieldset>
    ...
    <fieldset className="flex flex-col gap-3">
      ...
      <Radiobox name="tier" value="enterprise" disabled>Enterprise (unavailable)</Radiobox>
            ...
            >
                <Card title="Disabled">
                    <fieldset className="flex flex-col gap-3">
                        <legend className="mb-2 text-sm font-medium">Plan tier</legend>
