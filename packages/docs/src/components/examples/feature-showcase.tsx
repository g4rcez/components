    ...
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <Card title="Task Management">
                <TaskList>
                    {tasks.map((task) => (
                        <div key={task.id} className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-primary/10">
                            <Checkbox checked={task.completed} onChange={() => toggleTask(task.id)} asTask>
                                <span className={task.completed ? "text-muted decoration-muted" : ""}>{task.text}</span>
                            </Checkbox>
                        </div>
                    ))}
                </TaskList>
